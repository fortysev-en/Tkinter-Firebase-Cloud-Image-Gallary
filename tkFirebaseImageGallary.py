...
class StartFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        Label(self, text="GALLARY APP", fg="#282828", font=('ariel', 20, 'bold')).place(x=200, y=150)

        upload_btn = Button(self, text="UPLOAD IMAGES", font=('ariel', 10, 'bold'), fg="white", bg="green", command=lambda: controller.show_frame(UploadImages))
        upload_btn.place(x=85, y=250, width=205)

        view_btn = Button(self, text="VIEW IMAGES", font=('ariel', 10, 'bold'), fg="white", bg="green", command=lambda: controller.show_frame(ViewImages))
...
class UploadImages(tk.Frame):
    def selectFiles(self, controller):
        filez = fd.askopenfilenames(parent=self, title='Choose a file')
        ...
        for item in list(filez):
            if not (item.endswith('.png') or item.endswith('.jpg') or item.endswith('.jpeg') or item.endswith('.ico')):
                ...
                controller.selectedFiles.append(item)
                controller.status_label.config(text=f"{count} images selected.",fg="green")

    def uploadFiles(self, controller):
        controller.status_label.config(text=f"Uploading.... Please Wait!", fg="green")
        a = FirebaseConfig()

        for item in controller.selectedFiles:
            imageName = item.split('/')[-1]
            try:
                a.storage.child(f'FORTYSEVEN-IMAGE-GALLARY/{imageName}').put(item, controller.app_login_cred[
                    'idToken'].get())
            except Exception as e:
                controller.status_label.config(text=f"Error: {e}", fg="#c00000")

        controller.status_label.config(text=f"Images uploaded successfully!", fg="green")

    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)

        Label(self, text="UPLOAD IMAGES", fg="#282828", font=('ariel', 20, 'bold')).place(x=200, y=150)

        select_btn = Button(self, text="SELECT IMAGES", font=('ariel', 10, 'bold'), fg="white", bg="green", command=lambda: self.selectFiles(controller))
        select_btn.place(x=85, y=250, width=205)

        upload_btn = Button(self, text="UPLOAD IMAGES", font=('ariel', 10, 'bold'), fg="white", bg="green", command=lambda: threading.Thread(target=lambda: self.uploadFiles(controller),daemon=True).start())
        upload_btn.place(x=310, y=250, width=205)

        Label(self,
              text="Please note that, Images will be uploaded to your Cloud Firebase Account under a folder called FORTYSEVEN-IMAGE-GALLARY.",
              fg="#282828", font=('ariel', 10, 'bold'), wraplength=450).place(x=85, y=350)
